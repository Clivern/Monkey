...
class Job
{
     ...
     * @param int   $defaultRetry   The Default Number of Trials if Caller Failed
    ...
    public function __construct($callers = [], $defaultRetry = 1, $retryPerCaller = [])
    {
        ...
        $this->retryPerCaller = array_merge(
            array_fill_keys(array_keys($this->callers), $this->defaultRetry),
            $retryPerCaller
        );
    ...
    public function execute()
    {
        ...
        foreach ($this->callers as $ident => $caller) {
            $this->trialsPerCaller[$ident] = (isset($this->trialsPerCaller[$ident])) ?
                $this->trialsPerCaller[$ident] : 0;
            $this->retryPerCaller[$ident] = (isset($this->retryPerCaller[$ident])) ?
                $this->retryPerCaller[$ident] : $this->defaultRetry;

            if ($caller->getStatus() === CallerStatus::$SUCCEEDED) {
                continue;
            }

            if (($caller->getStatus() === CallerStatus::$FAILED) &&
                ($this->trialsPerCaller[$ident] >= $this->retryPerCaller[$ident])
            ) {
     ...
     * @param string $ident the caller ident
     *
     * @return bool whether caller removed or not
     */
     ...
     * @param string $ident The caller ident
     *
     * @return bool whether caller exists or not
     */
     ...
     * @param string $ident The Caller Ident
     *
     * @return bool Whether all detected variables replaced or not
     */
    protected function updateCaller($ident)
    {
        ...
        foreach ($matches[0] as $match) {
            ...
            if ((isset($this->callers[$matchArr[0]])) &&
                ($this->callers[$matchArr[0]]->getStatus() === CallerStatus::$SUCCEEDED)
            ) {
                if (!($this->callers[$matchArr[0]]->itemExists($matchArr[1])) &&
                    !($this->callers[$matchArr[0]]->response()->itemExists($matchArr[1]))
                ) {
                    $status &= false;
                ...
                $dataToReplace[$match] = ($this->callers[$matchArr[0]]->itemExists($matchArr[1])) ?
                    ($this->callers[$matchArr[0]]->getItem($matchArr[1])) :
                    $this->callers[$matchArr[0]]->response()->getItem($matchArr[1]);
            } else {
                $status &= false;
